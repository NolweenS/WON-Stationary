<nav x-data="{ open: false }" class="bg-[#FDFBF7] border-b border-[#EAE5DE]">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-20"> {{-- Hoogte iets verhoogd voor het tekstlogo --}}
            <div class="flex">
                {{-- LOGO: WON-Stationary tekst linkt naar Home --}}
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('home') }}" class="flex flex-col">
                        <span class="font-serif text-2xl text-[#3E2C22] tracking-widest font-bold uppercase">
                            WON-Stationary
                        </span>
                        <span class="text-[9px] tracking-[0.3em] text-[#8C7B70] uppercase -mt-1">
                            Elegantie & Stijl
                        </span>
                    </a>
                </div>

                {{-- NAVIGATIE LINKS IN HET HOOFDMENU --}}
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="route('cart.index')" :active="request()->routeIs('cart.index')" class="relative group">
                        ...
                        @if($cartCount > 0)
                            <span class="absolute top-4 -right-2 bg-[#8C7B70] text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">
                                {{ $cartCount }}
                            </span>
                        @endif
                    </x-nav-link>
                    ...
                    <div class="relative mr-4" x-data="{ open: false }">
                        <button @click="open = !open" class="relative p-1 rounded-full text-[#8C7B70] hover:text-[#3E2C22] focus:outline-none transition">
                            ...
                            @if(Auth::user()->unreadNotifications->count() > 0)
                                <span class="absolute top-0 right-0 inline-flex items-center justify-center px-1.5 py-0.5 text-[10px] font-bold leading-none text-white transform translate-x-1/4 -translate-y-1/4 bg-[#8C7B70] rounded-full">
